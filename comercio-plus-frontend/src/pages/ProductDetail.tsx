...
interface ApiProduct {
  ...
  store: { id: number | string; name: string; slug?: string } | null
}
...
export default function ProductDetail() {
  ...
  const handleAddToCart = () => {
    if (!product) return
    const sid = String(product.store_id ?? product.store?.id ?? '')
    const sname = toSafe(product.store?.name, 'ComercioPlus')
    addToCart(
      {
