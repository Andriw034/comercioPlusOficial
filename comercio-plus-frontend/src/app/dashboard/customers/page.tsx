...
const CUSTOMERS_REFRESH_INTERVAL_MS = 15000
...
export default function DashboardCustomers() {
  ...
  const fetchCustomers = useCallback(async (silent = false) => {
    try {
      ...
      const { data } = await API.get<ApiResponse>('/merchant/customers', {
        // Fuerza respuesta fresca para reflejar nuevos clientes registrados.
        params: { _t: Date.now() },
      })
  ...
  const handleDeleteCustomer = async (customerId: number, name: string) => {
    if (deletingIds.includes(customerId)) return

    const confirmed = window.confirm(`Eliminar a ${name} del directorio de clientes?`)
    if (!confirmed) return
                      ...
                      <tr key={item.id} className="border-b border-slate-100 last:border-b-0 hover:bg-slate-50/70 dark:border-white/5">
                        ...
                        <td className="px-4 py-3.5">
                          <ErpBtn
                            ...
                            onClick={() => void handleDeleteCustomer(item.id, name)}
                          >
                            {deletingIds.includes(item.id) ? 'Eliminando...' : 'Eliminar'}
                          </ErpBtn>
                        </td>
                      </tr>
                    )
