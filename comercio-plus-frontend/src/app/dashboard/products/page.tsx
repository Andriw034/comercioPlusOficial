      ...
      <div className="h-10 w-10 overflow-hidden rounded-lg border border-[#E2E8F0] bg-white">
        {showImage ? (
          ...
          <div className="flex h-full w-full items-center justify-center bg-[#F8FAFC]">
            <div className="flex h-6 w-6 items-center justify-center rounded-md border border-[#CBD5E1] bg-white text-[#94A3B8] shadow-[0_1px_2px_rgba(15,23,42,0.08)]">
              <Icon name="image" className="h-3.5 w-3.5" />
            </div>
            <span className="sr-only">{fallbackEmoji}</span>
          </div>
        )}
      </div>
...
export default function ManageProducts() {
  ...
  useEffect(() => {
    ...
    const onStorage = (event: StorageEvent) => {
      if (event.key !== 'inventory:last-import-at' || !event.newValue) return
      syncProductsAfterImport()
    }

    window.addEventListener('inventory:imported', onInventoryImported as EventListener)
    window.addEventListener('storage', onStorage)

    return () => {
      window.removeEventListener('inventory:imported', onInventoryImported as EventListener)
              ...
              <form id="product-create-form" className="space-y-4" onSubmit={save}>
                <section className="rounded-2xl border border-[#E2E8F0] bg-white p-4 shadow-[0_8px_20px_rgba(15,23,42,0.05)]">
                  <div className="mb-3 flex items-center gap-2 border-b border-[#E2E8F0] pb-2">
                    <span className="inline-flex h-7 w-7 items-center justify-center rounded-lg bg-[#FFF7ED] text-[#EA580C]">
