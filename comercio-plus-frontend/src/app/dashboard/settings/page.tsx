...
type TabId = 'profile' | 'taxes' | 'payments' | 'shipping' | 'notifications' | 'fiscal'
...
const DEFAULT_SETTINGS: Settings = {
  ...
  payments: [
    ...
    { id: 'card', name: 'Tarjeta credito / debito', description: 'Visa, Mastercard, Amex - procesado por MercadoPago', icon: '💳', is_active: true, requires_config: true },
...
export default function DashboardSettingsPage() {
  ...
  useEffect(() => {
    API.get('/profile')
      .then((r) => setProfile({ name: r.data.name, email: r.data.email }))
  ...
  return (
    <div className="space-y-6 pb-6">
      ...
      <div className="space-y-3">
        ...
        {/* Profile tab renders its own cards outside the shared wrapper */}
        {activeTab === 'profile' && (
          <div className="space-y-0">
            {/* Card — Datos personales */}
            <div
              className="relative rounded-2xl bg-white mb-4 p-6"
              style={{ boxShadow: '0 2px 12px 0 rgba(0,0,0,0.07), 0 1px 3px 0 rgba(0,0,0,0.05)', border: '1px solid #e5e7eb' }}
            >
              <div className="flex items-center gap-3 mb-5">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-500">Tu nombre e email de acceso al panel.</p>
                </div>
              </div>
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex items-center gap-3 border-t border-gray-100 pt-4">
                    <button
                      ...
                      className="bg-orange-500 hover:bg-orange-600 disabled:opacity-50 text-white font-medium px-5 py-2.5 rounded-lg text-sm transition shadow-sm active:scale-95"
                    >
                      {profileSaving ? 'Guardando...' : 'Guardar cambios'}
                    </button>
                    {profileSaved && <span className="text-sm font-medium text-green-600">✓ Cambios guardados</span>}
                    {profileError && <span className="text-sm font-medium text-red-500">{profileError}</span>}
                  </div>
                </div>
              )}
            </div>
            ...
            >
              <div className="flex items-center gap-3 mb-5">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-500">Mínimo 8 caracteres. Recomendado usar letras y números.</p>
                </div>
              </div>
              ...
              <div className="space-y-4">
                {(['current_password', 'password', 'password_confirmation'] as const).map((field) => {
                  ...
                  const showKey = field === 'current_password' ? 'current' : field === 'password' ? 'new' : 'confirm'
                    ...
                    <div key={field} className="space-y-1">
                      ...
                      <div className="relative">
                        ...
                        <input
                          ...
                          placeholder="••••••••"
                          className="w-full rounded-lg border border-gray-300 pl-9 pr-10 py-2.5 text-sm text-gray-900 placeholder-gray-400 outline-none transition focus:border-transparent focus:ring-2 focus:ring-orange-400"
                        />
                        <button
                          type="button"
                ...
                <div className="flex items-center gap-3 border-t border-gray-100 pt-4">
                  <button
                    ...
                    disabled={pwSaving || !passwords.current_password || !passwords.password || !passwords.password_confirmation}
                    className="disabled:opacity-40 text-white font-medium px-5 py-2.5 rounded-lg text-sm transition shadow-sm active:scale-95"
                    style={{ background: 'linear-gradient(135deg, #f97316 0%, #ef4444 100%)' }}
                    onMouseEnter={(e) => (e.currentTarget.style.background = 'linear-gradient(135deg, #ea6c0a 0%, #dc2626 100%)')}
                    onMouseLeave={(e) => (e.currentTarget.style.background = 'linear-gradient(135deg, #f97316 0%, #ef4444 100%)')}

                  >
                    {pwSaving ? 'Actualizando...' : 'Cambiar contraseña'}
                  </button>
                  {pwSaved && <span className="text-sm font-medium text-green-600">✓ Contraseña actualizada</span>}
                  {pwError && <span className="text-sm font-medium text-red-500">{pwError}</span>}
                </div>
              </div>
            </div>
          </div>
        ...
        {activeTab !== 'profile' && <SaveBar onSave={save} saving={saving} message={message} error={error} />}
      </div>
    </div>
  )
}
