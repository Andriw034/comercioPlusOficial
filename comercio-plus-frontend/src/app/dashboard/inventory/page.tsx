    ...
    <div className="space-y-5">
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <h1 className="font-display text-[32px] font-bold text-slate-950 dark:text-white">Inventario</h1>
          <p className="text-[12px] text-slate-500 dark:text-white/40">Control de stock en tiempo real</p>
        </div>

        <div className="flex items-center gap-2">
          <button
            onClick={() => load(true)}
            className="inline-flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-3 py-2 text-[12px] font-semibold text-slate-600 transition-colors hover:border-slate-300 hover:text-slate-900 dark:border-white/10 dark:bg-white/5 dark:text-white/70"
          >
            <RefreshCw size={14} /> Recargar
          </button>
          <Link
            to="/dashboard/inventory/receive"
            className="inline-flex items-center gap-2 rounded-xl bg-orange-500 px-3.5 py-2 text-[12px] font-semibold text-white transition-colors hover:bg-orange-600"
          >
            <ArrowUpCircle size={14} /> + Movimiento
          </Link>
        </div>
      </div>

      {!loadError && (totals.outProducts > 0 || totals.lowProducts > 0) && (
        <div className="flex flex-wrap items-center gap-3 rounded-2xl border border-rose-200 bg-rose-50 px-4 py-3 text-[13px] text-rose-700 dark:border-rose-500/20 dark:bg-rose-500/10 dark:text-rose-300">
          <AlertTriangle size={16} className="shrink-0" />
          <div className="flex-1">
            <p className="font-semibold">
              {totals.outProducts + totals.lowProducts} productos con stock critico
            </p>
          ...
          <button
            ...
            className="rounded-lg border border-rose-300 bg-white px-3 py-1.5 text-[12px] font-semibold text-rose-700 transition-colors hover:bg-rose-100 dark:border-rose-500/30 dark:bg-rose-500/10 dark:text-rose-300"
          >
            Reponer
          </button>
        </div>
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        <div className="rounded-2xl border border-slate-200 bg-white px-4 py-3 dark:border-white/10 dark:bg-white/5">
          <p className="text-[10px] uppercase tracking-[0.11em] text-slate-400 dark:text-white/30">Total</p>
        ...
        <div className="rounded-2xl border border-emerald-200 bg-emerald-50 px-4 py-3 dark:border-emerald-500/20 dark:bg-emerald-500/10">
          <p className="text-[10px] uppercase tracking-[0.11em] text-emerald-600/80 dark:text-emerald-300/70">Normal</p>
          <p className="mt-1 text-[30px] font-black text-emerald-700 dark:text-emerald-300">{totals.normalProducts}</p>
        </div>
        <div className="rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3 dark:border-amber-500/20 dark:bg-amber-500/10">
          <p className="text-[10px] uppercase tracking-[0.11em] text-amber-700/80 dark:text-amber-300/70">Bajo</p>
          <p className="mt-1 text-[30px] font-black text-amber-700 dark:text-amber-300">{totals.lowProducts}</p>
        </div>
        <div className="rounded-2xl border border-red-200 bg-red-50 px-4 py-3 dark:border-red-500/20 dark:bg-red-500/10">
          <p className="text-[10px] uppercase tracking-[0.11em] text-red-700/80 dark:text-red-300/70">Agotado</p>
          <p className="mt-1 text-[30px] font-black text-red-700 dark:text-red-300">{totals.outProducts}</p>
        </div>
      </div>
      ...
      {loadError && (
        <div className="flex items-center gap-2 rounded-2xl border border-red-200 bg-red-50 px-4 py-3 text-[13px] text-red-700 dark:border-red-500/20 dark:bg-red-500/10 dark:text-red-300">
          <AlertCircle size={16} />
          <span>{loadError}</span>
          <button onClick={() => load(true)} className="ml-auto text-[12px] font-semibold underline">
            Reintentar
          </button>
        </div>
      )}

      <GlassCard className="overflow-hidden border border-slate-200 p-0 shadow-[0_8px_24px_rgba(15,23,42,0.06)] dark:border-white/10">
        <div className="flex flex-wrap items-center gap-3 border-b border-slate-100 px-4 py-3.5 dark:border-white/10">
          ...
          <div className="relative">
            ...
            >
              ...
              <option value="deficit">Mayor deficit</option>
            </select>
          </div>
        </div>
            ...
            <table className="w-full min-w-[900px]">
              <thead>
                <tr className="border-b border-slate-100 bg-slate-50/60 dark:border-white/5 dark:bg-white/5">
                  ...
                  <th className="px-4 py-2.5 text-left text-[10px] font-semibold uppercase tracking-[0.11em] text-slate-400 dark:text-white/30">Min/Max</th>
              ...
              <tbody>
                {displayed.map((item) => {
                  ...
                  const stockRatio = item.min_stock > 0
                  ...
                  return (
                    <tr key={item.product_id} className="border-b border-slate-100 transition-colors hover:bg-slate-50/80 dark:border-white/5 dark:hover:bg-white/5">
                      ...
                      <td className="px-4 py-3 text-[12px] text-slate-600 dark:text-white/60">
                        {item.min_stock}/{Math.max(item.min_stock * 2, item.current_stock || item.min_stock)} {item.unit}
                      </td>
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-1.5">
                          <button
                            ...
                            className="inline-flex items-center gap-1 rounded-lg border border-orange-200 bg-orange-50 px-2.5 py-1 text-[11px] font-semibold text-orange-700 transition-colors hover:bg-orange-100 dark:border-orange-500/30 dark:bg-orange-500/10 dark:text-orange-300"
                          >
                            <SlidersHorizontal size={12} /> Ajustar
                          </button>
                          <button
                            onClick={() => setKardexItem(item)}
                            className="inline-flex items-center gap-1 rounded-lg border border-slate-200 bg-white px-2.5 py-1 text-[11px] font-semibold text-slate-600 transition-colors hover:border-slate-300 dark:border-white/10 dark:bg-white/5 dark:text-white/60"
                          >
        ...
        {displayed.length > 0 && (
          <div className="flex flex-wrap items-center justify-between gap-2 border-t border-slate-100 px-4 py-3 text-[12px] text-slate-500 dark:border-white/5 dark:text-white/40">
            ...
            <span className="inline-flex items-center gap-1">
              <Boxes size={12} /> Valor total: {fmtCurrency(totals.inventoryValue)}
            </span>
          </div>
        )}
      </GlassCard>
