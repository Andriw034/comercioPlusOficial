    ...
    <div class="max-w-md w-full space-y-8 cp-card p-8">
      <div>
        ...
        <p class="mt-2 text-center text-sm text-gray-300">
          O
          <router-link to="/login" class="font-medium text-[#EE471B] hover:text-[#d63f18]">
            iniciar sesión
          </router-link>
        </p>
      </div>
      <form class="mt-8 space-y-6" @submit.prevent="handleSubmit">
        ...
        <div v-if="error" class="bg-[#2A0F0F] border border-red-700 text-red-200 px-4 py-3 rounded">
          {{ error }}
        </div>
