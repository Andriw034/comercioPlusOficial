...
{
    public const IMPORTER_VERSION = '2026-03-11.1';
    ...
    private const STANDARD_FIELD_ALIASES = [
        ...
        'sku' => ['sku', 'codigo', 'codigo_sku', 'codigo_de_sku', 'product_code', 'code', 'codigosku', 'id_producto', 'referencia', 'ref', 'codigo_producto', 'codigo_del_producto', 'codigo interno'],
    ...
    /**
     * Caché intra-importación: evita múltiples DB-roundtrips cuando el mismo
     * nombre de categoría aparece en cientos de filas del mismo archivo.
     * Clave: "{store_id}:{lowercase_name}"  →  Valor: Category
     *
     * Se reinicia en cada llamada a import() o preview() para no contaminar
     * importaciones sucesivas en el mismo ciclo de vida del proceso.
     *
    ...
    public function preview(Store $store, UploadedFile $file): array
    {
        ...
        Log::info('[inventory-import] preview started', [
            'store_id'  => $store->id,
        ...
        Log::info('[inventory-import] preview parsed', [
            'store_id' => (int) $store->id,
            'rows' => $rows->count(),
            'parse_meta' => $this->lastParseMeta,
        ]);
        if ($rows->isEmpty()) {
            return [
                'headers' => [],
    ...
    public function import(Store $store, UploadedFile $file, bool $upsert = true): array
    {
        ...
        Log::info('[inventory-import] import started', [
            'store_id'  => $store->id,
            'file_name' => $file->getClientOriginalName(),
            'file_size' => $file->getSize(),
            'mime'      => $file->getMimeType(),
        ...
        $rows = $this->parseFile($file);
        Log::info('[inventory-import] import parsed', [
            'store_id' => (int) $store->id,
            'rows' => $rows->count(),
            'parse_meta' => $this->lastParseMeta,
        ]);
        ...
        try {
            ...
            DB::rollBack();
            Log::error('[inventory-import] import transaction failed', [
                'message' => $e->getMessage(),
                'class'   => get_class($e),
                'file'    => $e->getFile(),
                'line'    => $e->getLine(),
    ...
    {
        Log::info('[inventory-import] parsing CSV', [
        ...
        if ($handle === false) {
            throw new \RuntimeException('No se pudo abrir el archivo CSV. Ruta: ' . $file->getRealPath());
        }
        ...
        $this->lastParseMeta['delimiter'] = $delimiter;
        $this->lastParseMeta['row_count'] = $rows->count();
        $this->lastParseMeta['column_count'] = (int) (($rows->first()?->count()) ?? 0);
        Log::info('[inventory-import] CSV parsed', [
            'row_count' => $rows->count(),
            'column_count' => $this->lastParseMeta['column_count'],
            'delimiter' => $delimiter,
        ]);
    ...
    private function parseSpreadsheet(UploadedFile $file): Collection
    {
        ...
        try {
            ...
            Log::info('[inventory-import] loading spreadsheet', [
                'path' => $file->getRealPath(),
                'size' => $file->getSize(),
            ]);
            ...
            Log::info('[inventory-import] spreadsheet parsed', ['row_count' => $rows->count()]);
            $this->lastParseMeta['row_count'] = $rows->count();
            $this->lastParseMeta['column_count'] = (int) (($rows->first()?->count()) ?? 0);

            return $rows;
        } catch (\Throwable $e) {
            $this->lastParseMeta['error_class'] = get_class($e);
            Log::error('[inventory-import] parseSpreadsheet failed', [
                'message' => $e->getMessage(),
                'class'   => get_class($e),
                'file'    => $e->getFile(),
                'line'    => $e->getLine(),
            ...
            ini_set('memory_limit', $prevMemoryLimit ?: '128M');
        }
    }
    ...
    private function findOrCreateCategory(string $categoryName, Store $store): Category
    {
        ...
        $storeId = (int) $store->id;
        $cacheKey = $storeId . ':' . Str::lower($categoryName);

        // 1. Caché intra-importación: si ya resolvimos esta categoría en la misma
        //    ejecución, la devolvemos directamente sin tocar la DB.
        if (isset($this->categoryCache[$cacheKey])) {
            return $this->categoryCache[$cacheKey];
        }

        // 2. Generar un slug que sea único dentro de la tienda.
        //    Si el slug base ya existe en la tienda (misma tienda, diferente nombre),
        //    agregamos un sufijo numérico hasta encontrar uno libre.
        ...
        // 3. firstOrCreate con (slug, store_id) como clave de búsqueda/inserción.
        //    Esto es atómico y aislado por tienda — nunca pisa datos de otro comerciante.
        $category = Category::firstOrCreate(
            ['slug' => $slug, 'store_id' => $storeId],
            ['name' => $categoryName, 'store_id' => $storeId],
        );

        // 4. Si firstOrCreate encontró una con el mismo slug pero distinto nombre,
        //    buscamos por nombre exacto (búsqueda de respaldo case-insensitive).
        if (Str::lower($category->name) !== Str::lower($categoryName)) {
