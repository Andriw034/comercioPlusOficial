...
@if($store && ($store->logo_path || $store->logo_url))
        <img src="{{ $store->logo_url ?? asset('storage/'.$store->logo_path) ?? asset('placeholder-product.png') }}" alt="Logo Tienda" class="h-10 w-10 rounded-2xl object-cover ring-1 ring-white/10" loading="lazy" referrerpolicy="no-referrer" onerror="this.src='{{ asset('placeholder-product.png') }}'" />
      @endif
      <div>
      <h1 class="text-4xl font-bold mb-6 flex items-center gap-2">
          {{ $store?->name ?? 'Mi Tienda' }}
          @if($store && ($store->cover_path || $store->cover_url))
  ...
  @if($store && $store->cover_path)
    <div class="relative mb-6 overflow-hidden rounded-3xl">
      <img src="{{ $store->cover_url ?? asset('storage/'.$store->cover_path) ?? asset('placeholder-product.png') }}" alt="Portada Tienda"
           class="w-full h-40 sm:h-52 object-cover opacity-90" loading="lazy" referrerpolicy="no-referrer" onerror="this.src='{{ asset('placeholder-product.png') }}'" />
