...
<div class="relative min-h-[calc(100vh-120px)] w-full overflow-hidden" style="--cp-primary:#FF6000; --cp-primary-2:#FF8A3D;">
  {{-- Fondo degradado estilo welcome --}}
  ...
  <div class="relative mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
    <div class="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
      <div>
        <h1 class="text-3xl font-extrabold text-white">Nuevo producto</h1>
        <p class="text-sm text-neutral-300">Anade un producto a tu catalogo de ComercioPlus</p>
      </div>
    ...
    <div class="rounded-2xl border border-white/10 bg-white/10 p-6 backdrop-blur-md">
      ...
      <form action="{{ route('admin.products.store') }}" method="POST" enctype="multipart/form-data" class="grid grid-cols-1 gap-6 lg:grid-cols-12">
        @csrf
        ...
        <div class="space-y-5 lg:col-span-7">
          ...
          <div class="grid grid-cols-1 gap-4 sm:grid-cols-3">
            ...
            <div>
              ...
              <input id="stock" type="number" step="1" min="0" name="stock" value="{{ old('stock') }}"
                     class="w-full rounded-lg border border-white/20 bg-white/10 px-3 py-2 text-white focus:outline-none focus:ring-2 focus:ring-[--cp-primary]">
            </div>
            <div>
          ...
          <div>
            ...
            <textarea id="description" name="description" rows="5"
                      class="w-full rounded-lg border border-white/20 bg-white/10 px-3 py-2 text-white placeholder:text-neutral-400 focus:outline-none focus:ring-2 focus:ring-[--cp-primary]">{{ old('description') }}</textarea>
          </div>

          <div class="flex items-center gap-3">
            <input id="status" type="checkbox" name="status" value="1" @checked(old('status', 1) == 1)
        ...
        <div class="lg:col-span-12 flex items-center justify-end gap-3 pt-2">
          ...
          <button type="submit" class="btn-cp">Guardar producto</button>
        </div>
      </form>
    </div>
  </div>
