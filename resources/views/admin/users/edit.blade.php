...
<div class="p-6 space-y-6">
    <form action="{{ route('admin.users.update', $user) }}" method="POST" enctype="multipart/form-data" class="space-y-4 bg-white/10 ring-1 ring-white/15 rounded-3xl p-6">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
                <label for="avatar" class="block text-sm font-medium text-white/90 mb-2">Avatar</label>
                @if($user->avatar)
                    <img src="{{ Storage::url($user->avatar) }}" alt="Avatar" class="w-16 h-16 rounded-full mb-2 ring-2 ring-white/20">
                @endif
        ...
        <div class="flex justify-end space-x-3 pt-4 border-t border-white/10">
            ...
            <button type="submit" class="px-6 py-2 text-black bg-orange-500 rounded-xl hover:bg-orange-600 font-semibold shadow smooth">Actualizar Usuario</button>
        </div>
    </form>
</div>
@endsection
