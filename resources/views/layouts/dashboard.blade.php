...
@section('content')
  {{-- Tarjetas de estado / accesos rápidos --}}
  ...
  {{-- Categorías mínimas (6) --}}
  <div class="mt-8">
    <h2 class="text-lg font-semibold">Categorías de repuestos y accesorios</h2>
    <p class="text-sm text-white/80">Organiza tus productos por categoría.</p>
    ...
    <div class="mt-4 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
      @foreach ($categorias as $cat)
        <a href="{{ route('admin.categories.index') }}"
           class="group rounded-2xl bg-white/10 ring-1 ring-white/15 px-5 py-4 hover:bg-white/15 transition">
          <div class="flex items-center justify-between">
            <div class="font-medium">{{ $cat }}</div>
            <span class="text-xs text-white/70 group-hover:text-white/90">Ver →</span>
          </div>
        </a>
      @endforeach
    </div>
